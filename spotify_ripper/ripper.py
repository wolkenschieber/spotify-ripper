...
class Ripper(threading.Thread):
    ...
    def search_query(self, query):
        ...
        for track_idx, track in enumerate(result.tracks):
            print("  " + Fore.YELLOW + str(track_idx + 1) + Fore.RESET + " [" + to_ascii(args, track.album.name) + "] "
                  + to_ascii(args, track.artists[0].name) + " - " + to_ascii(args, track.name)
                  + " (" + str(track.popularity) + ")")

        pick = input("Pick track(s) (ex 1-3,5): ")

        def get_track(i):
            if 0 <= i < len(result.tracks):
